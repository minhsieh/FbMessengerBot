...
if (!empty($_REQUEST['hub_mode']) && $_REQUEST['hub_mode'] == 'subscribe' && $_REQUEST['hub_verify_token'] == $verify_token) {
    ...
    if (!empty($data['entry'][0]['messaging'])) {
        ...
        foreach ($data['entry'][0]['messaging'] as $message) {
            ...
            $user = $farm->getUser($message['sender']['id']);
            ...
            // When bot receive button click from user
            	...
            	$postback['type'] = $postback[0];
            	$postback['payload'] = $postback[1] ?? "";
            ...
            if(!empty($postback)){
            	switch($postback['type']) {
	            	case "WELCOME":
	            		$bot->send(new Message($message['sender']['id'], "Welcome to M Farm ".$user['first_name']));
	            		break;
	            }
            }
            
            //Handle Postback First
            // 
            ...
            if(!empty($command)){
            	switch ($command) {
	                ...
	                case 'text':
	                    $bot->send(new Message($message['sender']['id'], 'This is a simple text message.'.$user['first_name']));
	                    break;
	                ...
	                case 'profile':
	                    ...
	                    $bot->send(new StructuredMessage($message['sender']['id'],
	                        ...
	                        [
	                            'elements' => [
	                                new MessageElement($user->getFirstName()." ".$user->getLastName(), "Hello I'm a very shy boy.", $user->getPicture())
	                            ],
	                            'image_aspect_ratio' => StructuredMessage::IMAGE_ASPECT_RATIO_SQUARE,
	                        ],
	                        [ 
	                ...
	                case 'set get started button':
	                    $result = $bot->setGetStartedButton('WELCOME');
	                    //$logger->debug(json_encode($result));
	                    break;
